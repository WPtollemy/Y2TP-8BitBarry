...
bool InvaderFormation::toShootOrNah() const
{
	...
	return (rand() % (this->shot_chance)) + 1 == 1;
}
...
void InvaderFormation::handleCollisions(Explosions &explosions, PlayerLaser &player_laser)
{
    for (unsigned i = 0; i < this->lasers.size(); ++i)
    {
        ...
        if (laser->checkCollide(player_laser.getShape().getGlobalBounds()))
        {
            ...
            if (laser->getType() == InvaderLaser::LaserType::NORMAL && (rand() % (3)) + 1 == 2)
                laser->setHit();
        }
    }
}
...
bool InvaderFormation::move(Player &player, PlayerLaser &player_laser, Pacman &pacman, LivesDisplay &lives_disp, Explosions &explosions)
{
        ...
        else
        {
            for (auto& invader_row : this->invaders)
            {
                for (auto& invader : invader_row)
                {
                    ...
                    {
                        player.die(*this, player_laser, pacman, explosions);
...
InvaderFormation::InvaderFormation(sf::RenderWindow &window, Textures &textures): window(window), move_tick(0), move_tick_max(MOVE_TICK_MAX_START), move_tick_change(MOVE_TICK_CHANGE_START), step_on(1), num_killed(0), has_hit_edge(false), shot_chance(SHOT_CHANCE_START)
{
...
void InvaderFormation::update(PlayerLaser &laser, Player &player, PlayerLaser &player_laser, Pacman &pacman, LivesDisplay &lives_disp, Explosions &explosions, unsigned &game_score)
{
    this->move(player, player_laser, pacman, lives_disp, explosions);
...
void InvaderFormation::setBarrierLine(const unsigned barrier_line)
{
    this->SHIELD_LINE = barrier_line;
}
