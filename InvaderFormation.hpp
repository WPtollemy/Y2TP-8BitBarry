...
private:
    ...
    bool move(Player &player, PlayerLaser &player_laser, Pacman &pacman, LivesDisplay &lives_disp, Explosions &explosions);
...
public:
    InvaderFormation(sf::RenderWindow &window, Textures &textures);
    ~InvaderFormation();
    ...
    void update(PlayerLaser &laser, Player &player, PlayerLaser &player_laser, Pacman &pacman, LivesDisplay &lives_disp, Explosions &explosions, unsigned &game_score);

    void draw(int amount = -1);
    void drawLasers();

    void setBarrierLine(const unsigned barrier_line);
