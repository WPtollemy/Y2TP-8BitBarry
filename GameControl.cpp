...
namespace edy{
	namespace pac{
		PacEngine::PacEngine():lives(3),cherryCount(0)
		{
		...
		bool PacEngine::loadMap(const std::string& path)
		{
			...
			int ghostpos=0;
			for(int y=0;y<31;++y) for(int x=0;x<28;++x)//28 instead of 56
			{
		...
		void PacEngine::update()
		{
			if(--cherryCount==0)
			{
				mapArray[cherryPos.x][cherryPos.y]=Empty;
			}
			if(cherryCount<=-60*10)
			{
				cherryPos=sf::Vector2i(rand()%28,rand()%31);
				if(mapArray[cherryPos.x][cherryPos.y]==Empty)
				{
					cherryCount=10*60;
					mapArray[cherryPos.x][cherryPos.y]=Cherry;
				}
			}
			updatePac();
		...
		void PacEngine::makeWallsMap(sf::RenderTarget& target)
		{
			...
			for(int x=0;x<57;++x)
			{
				if((mapArray[x][0]==Wall)&&(mapArray[x+1][0]==Wall)) drawHorizontalLine(target,x,x+1,0);
				if((mapArray[x][30]==Wall)&&(mapArray[x+1][30]==Wall)) drawHorizontalLine(target,x,x+1,30);
			}
			for(int y=0;y<30;++y)
			{
				if((mapArray[0][y]==Wall)&&(mapArray[0][y+1]==Wall)) drawVerticalLine(target,y,y+1,0);
				if((mapArray[28][y]==Wall)&&(mapArray[28][y+1]==Wall)) drawVerticalLine(target,y,y+1,28);
			}
			for(int x=1;x<28;++x) for(int y=1;y<30;++y)
			{
				if(mapArray[x][y]==Wall)
				{
					if(mapArray[x-1][y]==Wall) drawHorizontalLine(target,x-1,x,y);
					if(mapArray[x+1][y]==Wall) drawHorizontalLine(target,x+1,x,y);
					if(mapArray[x][y-1]==Wall) drawVerticalLine(target,y-1,y,x);
					if(mapArray[x][y+1]==Wall) drawVerticalLine(target,y+1,y,x);
				}
			}//for x for y
		}
		void PacEngine::updatePac()
		{
			...
			guys[0].position.x+=16*27;//as below, to avoid problems with negative numbers modulos
			guys[0].position.x%=16*27;//for tunnel purposes
			...
			if(guys[Pac].isAtNode())
			{
				...
				unsigned char & tmp=mapArray[t.x][t.y];
				switch(tmp)
				{
				case Cherry:
					tmp=Empty;
					eventList.push_back(PacEvent(EventType::ScoreChange,100));
					break;
				case Pill:
					tmp=Empty;
					eventList.push_back(PacEvent(EventType::ScoreChange,10));
					checkPills();
					break;
				case Booster:
					tmp=Empty;
					eventList.push_back(PacEvent(EventType::ScoreChange,50));
					ghostKill=0;//reset kill streak
		...
		void PacEngine::checkCollisions()
		{
			...
			for(int i=1;i<=4;++i)
			{
				if(guys[Pac].getNode()==guys[i].getNode())
				{
					...
					if(guys[i].scared>0)
					{
						eventList.push_back(PacEvent(EventType::ScoreChange,ghostKillPts(ghostKill)));
						guys[i].scared=-1;
					}
				}
			}
		...
		int PacEngine::fetchTileAt(sf::Vector2i pos,sf::Vector2i off)
		{
			...
			int y=pos.y+off.y;
			//tunnel special case
//			if(y==fitBetween(0,y,30)&&(x==-1||x==56)&&mMap[fitBetween(0,x,57)][y]==Tunnel) 
//				return Empty;
			//do not drive out of boundaries
			if(y!=fitBetween(0,y,30)||x!=fitBetween(0,x,57)) return Wall;
			return mapArray[x][y];
		}
		void PacEngine::checkPills()
		{
			int tmp=0;
			for(int y=0;y<31;++y) for(int x=0;x<27;++x)
			{
				if(mapArray[x][y]==Pill)++tmp;
			}
			if (tmp == 0 || skipping == true) {
				eventList.push_back(PacEvent(EventType::PillsExhausted, 0));}
			if(tmp<0.3f*totalPills)guys[Blinky].speed=3;
		}
