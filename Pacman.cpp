...
bool Pacman::toAppearOrNah()
{
	return (rand() % (this->APPEAR_CHANCE)) + 1 == 1;
}
...
void Pacman::handleHit(PlayerLaser &laser, unsigned &game_score)
{
...
Pacman::Pacman(Textures &textures): SCORES({50, 100, 150}), pacman_txtr(textures.UFO), explode_txtr(textures.INVADER_DEATH), on_screen(false), exploding(false), show_score(false), death_tick(DEATH_TICK_MAX), score_tick(SCORE_TICK_MAX), shots_fired(0)
{
}

void Pacman::reposition()
{
    ...
    this->sprite.setTexture(this->pacman_txtr, true);
...
void Pacman::update(PlayerLaser &laser, unsigned &game_score)
{
