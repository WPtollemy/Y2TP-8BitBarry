...
namespace edy{
	namespace state{
			...
			while(mapOk)
			{
				...
				while(mEngine.getEvent(peve))
				{
					...
					/*case pac::EventType::GameOver:
						...
						std::cout << "Game Over";
						//pack.Manager->~StateManager();
						pack.Manager->clearStates();
						return;
						break;/**/
					case pac::EventType::PacDied:
						lives.setString(sup::toString(peve.data)+" Lives");
						break;
		...
		void GameState::drawAll()	//*****************************************************************Map stuff here
		{
			...
			for(int x=0;x<47;++x)for(int y=0;y<31;++y)
			{
