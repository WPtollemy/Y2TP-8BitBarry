...
Barrier::Barrier(Textures &textures, const int xpos, const int ypos): textures(textures)
{
    this->reset();
...
void Barrier::handleCollide(PlayerLaser &player_laser, Lasers &invader_lasers, Explosions &explosions)
{
    // TODO: Make code DRYer by figuring out a way to not repeat pixel FOR loop.

    sf::FloatRect barrier_rect = this->sprite.getGlobalBounds();
    unsigned xpos = this->sprite.getPosition().x;
    unsigned ypos = this->sprite.getPosition().y;
    unsigned width = barrier_rect.width;
    unsigned height = barrier_rect.height;

    // Check for invader laser collision
    for (auto& laser : invader_lasers)
    {
        // First check if laser rect intersects with barrier rect
        if (laser->checkCollide(barrier_rect))
        {
            ...
            for (unsigned x = xpos; x < (xpos + width); ++x)
            {
                for (unsigned y = ypos; y < (ypos + height); ++y)
                {
                    ...
                    if (pixel.g > 0 && laser->checkCollide(x, y))
                    {
                        ...
                        laser->remove();
                        this->damageBarrier(x - xpos, y - ypos, laser->getDmg());
    ...
    if (player_laser_rect.intersects(barrier_rect))
    {
        // Check to see if pixel collides with any Invader lasers.
        // Only true if hit green pixel
        for (unsigned x = xpos; x < (xpos + width); ++x)
        {
            for (unsigned y = ypos; y < (ypos + height); ++y)
            {
                ...
                {
                    this->damageBarrier(x - xpos, y - ypos, player_laser.getDmg());
                    player_laser.stop();
                }
            }
        }
...
void Barrier::handleCollideInvaders(InvaderFormation &invaders)
{
    sf::FloatRect barrier_rect = this->sprite.getGlobalBounds();
    unsigned xpos = this->sprite.getPosition().x;
    unsigned ypos = this->sprite.getPosition().y;
    ...
    for (auto& invader_row : invaders.getInvaders())
    {
        ...
        {
            // First check if invader rect intersects with barrier rect
            if (invader->getSprite().getGlobalBounds().intersects(barrier_rect))
            {
...
void Barrier::damageBarrier(int x, int y, const int dmg)
{
    int oldx = x;
    int oldy = y;
