...
namespace edy{
	namespace pac{
		...
		bool PacEngine::loadMap(const std::string& path)
		{
			...
			int ghostpos=0;
			for(int y=0;y<31;++y) for(int x=0;x<47;++x)//28 instead of 56
			{
		...
		void PacEngine::update()
		{
			...
			{
				cherryPos=sf::Vector2i(rand()%47,rand()%31);
		...
		void PacEngine::makeWallsMap(sf::RenderTarget& target)
		{
			...
			for(int y=0;y<30;++y)
			{
				...
				if((mMap[48][y]==Wall)&&(mMap[48][y+1]==Wall)) drawVerticalLine(target,y,y+1,48);
			}
			for(int x=1;x<48;++x) for(int y=1;y<30;++y)
			{
				if(mMap[x][y]==Wall)
				{
		...
		void PacEngine::updatePac()
		{
			...
			guys[0].position.x+=16*47;//as below, to avoid problems with negative numbers modulos
			guys[0].position.x%=16*47;//for tunnel purposes
